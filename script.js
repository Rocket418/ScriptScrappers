...
const acraPol = [
  "http://localhost:5000/acraPol1",
  "http://localhost:5000/acraPol2",
  "http://localhost:5000/acraPol3",
];
...
async function reorganize(variable) {
  ...
  let fixedAnswers = [];
  console.log(infoScrapper);
  for (infoScrap of infoScrapper) {
    scraps.push(infoScrap);
    if (
      scraps[3] &&
      !scraps[3]["SIMULACROS 2022 EN PROCESO DE ACTUALIZACIÓN"].length
    ) {
      scraps.pop();
    }
            ...
            if (used === dePol) {
              for (test in topic) {
                for (questionObject of topic[test]) {
                  ...
                  let realQuestion = questionObject.question.replace(
                    /<\/?[^>]+(>|$)/g,
                    ""
                  );
                  ...
                  function correctAnswer() {
                    for (string of answerObject.answers) {
                      if (
                        !!questionObject.rightanswer
                          .toString()
                          .includes(string.goodAnswer[1])
                      ) {
                        return string.id;
                      }
                    }
                ...
                if (test.includes("INTRO") || test.includes("NIVEL 1")) {
                  ...
                  test.includes("NIVEL 2") ||
                  test.includes("NIVEL 3")
                ) {
                  for (fixedAnswer of fixedAnswers) {
                    if (test === fixedAnswer.Origin) {
                      let index = {
              ...
              let realQuestion = topic.question
                .replace(/<\/?[^>]+(>|$)/g, "")
                .split("-");
  ...
  if (used === variable) {
    fs.writeFile(
      "reorganicedScrapperofiPol.json",
